...
### 渲染
#### 2D渲染
对于绝大多数的渲染，浏览器引擎能正常的进行解析和显示，不过对于CSS3中的一些2D渲染，主要值得注意
#### 3D渲染
...
## 兼容性指南
*	浏览器分级标准
*	fixed有兼容性问题，android2.2-以及ios4.0-不支持,ios5支持不完整
*	兼容性需要考虑不同的机器的开发商，特别要注意国内的二次深度定制的厂商，比如阿里云，小米，魅族，锤子手机等

## 容器使用指南
* jsapi注入是有时间的，因此在使用api的时候，需要判断bridge对象是否存在，如果不存在，需要加入到ready的函数里。
* jsapi接口有权限限制
